...
def retrieve_recommendations(request,pk):
	if request.method == 'GET':
		try:
...
def create_recommendations(request):
	if request.method == 'POST':
		try:
			...
			return JsonResponse("Meal does not exist",safe=False)
		if len(recommendationList) == 0:
			recommendation = Recommendation(item_id = request.POST['item_id'], recommended_items = request.POST['recommended_items']+",")
			recommendation.save()
		else:
			recommendationList[0].recommended_items = recommendationList[0].recommended_items + request.POST['recommended_items']+","
			recommendationList[0].save()
		return JsonResponse("Successfully created the recommended item",safe=False)
	else:
		return JsonResponse("Must Post",safe=False)
...
def recommendations(request):
    if request.method == "POST":
        ...
        return JsonResponse("Only accept POST requests",safe=False)